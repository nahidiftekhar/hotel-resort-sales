    ...
    <>
      <Container className="px-0 mt-5 min-vh-50">
        <div className="add-action">
          <Dropdown>
            <Dropdown.Toggle id="add-action" className="btn btn-one pe-2 mb-3 mx-3">
              <span className="mx-2">Add action</span>{" "}
              <Icon nameIcon="FaPlus" propsIcon={{ size: 16 }} />
            </Dropdown.Toggle>

            <Dropdown.Menu className="dropdown-menu-custom" variant="dark">
              {actionsList.map(({ actionTypeId, actionName, actionIcon }) => (
                <Dropdown.Item
                  key={actionTypeId}
                  href="#"
                  className="btn btn-three"
                  onClick={() => handleActionClick(actionTypeId, 1, 0)}
                >
                  {actionName}
                  {actionIcon ? <Icon nameIcon={actionIcon} propsIcon={{ size: 20 }} /> : ""}
                </Dropdown.Item>
              ))}
            </Dropdown.Menu>
          </Dropdown>

          <a href={`${group_link}${card_link}`} target='_blank' rel="noreferrer" >
            <Button className="btn btn-one pe-2 mb-3 mxe-3">
              <span className="mx-2">Preview</span>{" "}
              <Icon nameIcon="FaEye" propsIcon={{ size: 16 }} />
            </Button>
          </a>
        </div>

        {allAboutCard.length===1 && !allAboutCard[0].action_id ? 
        <p className="text-center my-5">You haven&apos;t added any action yet.</p> : 
