  ...
  const headerResponsive = [
    ...
    },
    {
      ...
      selector: (row) => (
        <div>
          <div>
            {BDTFormat.format(row.price_adult)}
            <span className="smaller-label"> {row.unit}</span>
          </div>
          {Number(row.price_kids) > 0 && (
            <div>
              {BDTFormat.format(row.price_kids)}
              <span className="smaller-label"> {row.unit_kids}</span>
            </div>
          )}
        </div>
      ),
