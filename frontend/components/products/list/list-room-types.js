...
import { listAllRoomsApi, listRoomTypesApi } from '@/api/products-api';
...
function ListRoomTypes() {
  ...
  useEffect(() => {
    const fetchProductList = async () => {
      const allProductList = await listRoomTypesApi();
  ...
  const headerResponsive = [
    ...
    },
    {
      ...
      selector: (row) => (
        <div>
          <div>Adult: {row.max_adult}</div>
          <div>Child: {row.max_child}</div>
        </div>
      ),
      sortable: true,
      wrap: true,
